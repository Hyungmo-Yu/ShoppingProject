...
public class ProductDAO
{
	...
	private static final String ALLRETRIEVE_STMT = "SELECT * FROM c##shoppingproduct";
	private static final String INSERT_STMT = "INSERT INTO c##shoppingproduct VALUES(?,?,?,?,?,?)";
	private static final String UPDATE_STMT = "UPDATE c##shoppingproduct SET ProductType = ?, ProductName = ?, Explanation = ?, Price = ?, Inventory = ? WHERE ProductID = ?";
	private static final String GETID_STMT = "SELECT COUNT(ProductID) FROM c##shoppingproduct";
	...
	ArrayList<Product> productRetrieve(String productname) throws SQLException
	{
		...
		{
			conn = connPool.getPoolConnection();
			stmt = conn.prepareStatement("SELECT * FROM c##shoppingproduct WHERE ProductName like '%" + productname + "%'");
			rset = stmt.executeQuery();
			while (rset.next())
			{
				int ProductID = rset.getInt(1);
